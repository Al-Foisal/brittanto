...
@section('title') {{ auth()->user()->abbreviation . ' Section Correction'}} @stop
			...
			<div id="container_demo" >
				...
				<div id="wrapper">
					<div class="animate form">
						...
						<form  action="{{ route('coaching-sections.update',$section) }}" method="post"> 
							@method('PUT')
							@csrf
							<h1>{{ auth()->user()->abbreviation . ' Section Correction'}}</h1> 
							<div style="width: 45%;float: left;margin-right: 5%;">
								<p> 
									<label for="name"> Batch Numer: </label>
									<input id="name" value="{{ $section->name }}" readonly required="required" type="string" step="0.01" disabled/>
								</p>
							</div>
							...
							<div style="width: 48%;float: left;">
								<p>
									...
									<select id="type" type="text" name="gender">
										<option value="{{ $section->gender }}" selected>{{ $section->gender }}</option>
							...
							<div style="width: 45%;float: left;margin-right: 5%;">
								<p>
									...
									<select id="class" type="text" name="class" disabled>
										
										<option value="{{ $section->class }}" selected>{{ $section->class_type }}</option>
							...
							<div style="width: 48%;float: left;">
								<p>
									...
									<select id="type" type="text" name="type">
										<option value="{{ $section->type }}" selected>{{ $section->section_type }}</option>
							...
							<div style="width: 45%;float: left;margin-right: 5%;">
								<p> 
									...
									<input id="start_time" value="{{ date('h:i',strtotime($section->start_time)) }}" name="start_time" required="required" type="time"/>
								</p> 
							</div>
							...
							<div style="width: 48%;float: left;">
								<p> 
									...
									<input id="end_time" value="{{ $section->end_time ? date('H:i',strtotime($section->end_time)) : '' }}" name="end_time" required="required" type="time"/>
								</p> 
							</div>

							<p> 
								<label for="hint"> Hint </label>
								<textarea id="hint" name="hint" required="required" rows="2" cols="60">{{ $section->hint }}</textarea>
							</p>							
